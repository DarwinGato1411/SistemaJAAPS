...
//                        
            ...
            linea = (" <TipoIDInformante>R</TipoIDInformante>\n"
                    ...
                    + " <razonSocial>" + amb.getAmRazonSocial() + "</razonSocial>\n"
            ...
            for (CabeceraCompra item : detalleCompra) {
                ...
                linea = ("        <detalleCompras>\n"
                        + "            <codSustento>06</codSustento>\n"
                        + "            <tpIdProv>" + item.getIdProveedor().getIdTipoIdentificacionCompra().getTicCodigo() + "</tpIdProv>\n"
                        + "            <idProv>" + item.getIdProveedor().getProvCedula() + "</idProv>\n"
                        + "            <tipoComprobante>01</tipoComprobante>\n"
                        + "            <parteRel>NO</parteRel>\n"
                        + "            <fechaRegistro>" + formato.format(item.getCabFecha()) + "</fechaRegistro>\n"
                        + "            <establecimiento>" + amb.getAmEstab() + "</establecimiento>\n"
                        + "            <puntoEmision>" + amb.getAmPtoemi() + "</puntoEmision>\n"
                        + "            <secuencial>" + item.getCabNumFactura() + "</secuencial>\n"
                        + "            <fechaEmision>" + formato.format(item.getCabFecha()) + "</fechaEmision>\n"
                        + "            <autorizacion>" + item.getCabAutorizacion() + "</autorizacion>\n"
                        + "            <baseNoGraIva>" + BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP) + "</baseNoGraIva>\n"
                        + "            <baseImponible>" + BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP) + "</baseImponible>\n"
                        + "            <baseImpGrav>" + item.getCabSubTotal().setScale(2, RoundingMode.HALF_UP) + "</baseImpGrav>\n"
